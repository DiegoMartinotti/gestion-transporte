    ...
    chofer: {
        ...
        validate: {
            validator: function (value) {
                return mongoose.model('Personal').findById(value).lean().then((personal) => {
        ...
        if (this.isNew || tempTariffInfo || this.isModified('origen') || this.isModified('destino') ||
            ...
            if (tempTariffInfo) {
                ...
                if (!tramo) {
                    ...
                    const origenPopulated = this.populated('origen') ? this.origen : await Site.findById(viaje.origen);
                    const destinoPopulated = this.populated('destino') ? this.destino : await Site.findById(viaje.destino);
                    const origenNombre = origenPopulated?.Site || 'ID desconocido';
                    const destinoNombre = destinoPopulated?.Site || 'ID desconocido';
                    throw new Error(`No se encontró un tramo válido para Cliente ${clienteDoc.nombre} (${viaje.cliente}) desde ${origenNombre} hasta ${destinoNombre} para la fecha ${viaje.fecha.toISOString().split('T')[0]}`);
                }
                ...
                if (!tarifaVigente) {
                    ...
                    throw new Error(`No se encontró una tarifa (${viaje.tipoTramo}) vigente para tramo ${origenNombre} → ${destinoNombre} (Cliente: ${clienteDoc.Cliente}) en fecha ${viaje.fecha.toISOString().split('T')[0]}`);
                }
            ...
            switch (tarifaVigente.metodoCalculo) {
                case 'Palet':
                    ...
                    if (formulaPersonalizada) {
                        logger.info(`Usando fórmula personalizada para ${clienteDoc.Cliente} (${viaje.tipoUnidad}): ${formulaPersonalizada}`);
