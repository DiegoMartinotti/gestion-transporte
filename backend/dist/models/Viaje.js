        ...
        validate: {
            validator: function (value) {
                return __awaiter(this, void 0, void 0, function* () {
                    const personal = yield Personal.findById(value).lean();
        ...
        try {
            ...
            if (this.isNew || this.isModified('vehiculos')) {
                if (this.vehiculos && this.vehiculos.length > 0) {
                    const vehiculoPrincipal = yield Vehiculo.findById(this.vehiculos[0].vehiculo).lean();
            ...
            if (this.isNew || tempTariffInfo || this.isModified('origen') || this.isModified('destino') ||
                ...
                const clienteDoc = yield Cliente.findById(this.cliente);
                if (!clienteDoc)
                ...
                if (tempTariffInfo) {
                    ...
                    if (tarifaVigente.metodoCalculo === 'Kilometro') {
                        ...
                        if (typeof tramo.distancia === 'undefined' || tramo.distancia === null || tramo.distancia <= 0) {
                            ...
                            const tramoEncontrado = yield Tramo.findById(tarifaVigente.tramoId).lean();
                    ...
                    if (!tramo) {
                        yield this.populate('origen destino');
                        const origenNombre = ((_a = this.origen) === null || _a === void 0 ? void 0 : _a.nombre) || 'ID desconocido';
                        const destinoNombre = ((_b = this.destino) === null || _b === void 0 ? void 0 : _b.nombre) || 'ID desconocido';
                    ...
                    if (!tarifaVigente) {
                        yield this.populate('origen destino');
                        const origenNombre = ((_c = this.origen) === null || _c === void 0 ? void 0 : _c.nombre) || 'ID desconocido';
                        const destinoNombre = ((_d = this.destino) === null || _d === void 0 ? void 0 : _d.nombre) || 'ID desconocido';
                ...
                switch (tarifaVigente.metodoCalculo) {
                    case 'Palet':
                        ...
                        if (formulaPersonalizada) {
                            ...
                            if (isNaN(valorTarifaParaFormula))
                                throw new Error('Valor de tarifa inválido para fórmula Palet.');
                            ...
                            if (isNaN(valorTarifa))
                                throw new Error('Valor de tarifa inválido para cálculo Palet.');
                    ...
                    case 'Kilometro':
                        ...
                        if (!tramo || typeof tramo.distancia === 'undefined' || tramo.distancia === null || tramo.distancia <= 0) {
                            logger.warn(`[TARIFA] El tramo ID ${(tramo === null || tramo === void 0 ? void 0 : tramo._id) || (tempTariffInfo === null || tempTariffInfo === void 0 ? void 0 : tempTariffInfo.tramoId)} no tiene distancia válida (${tramo === null || tramo === void 0 ? void 0 : tramo.distancia}) para cálculo por Km.`);
                            tarifaBase = 0; // O lanzar error si la distancia es obligatoria
                        }
                        else {
                            const valorTarifa = Number(tarifaVigente.valor);
                            if (isNaN(valorTarifa))
                                throw new Error('Valor de tarifa inválido para cálculo Km.');
                    ...
                    case 'Fijo':
                        const valorTarifa = Number(tarifaVigente.valor);
                        if (isNaN(valorTarifa))
                            throw new Error('Valor de tarifa inválido para cálculo Fijo.');
            ...
            if (this.extras && this.extras.length > 0) {
                ...
                let extrasPoblados = this.populated('extras.extra') || (((_e = this.extras[0]) === null || _e === void 0 ? void 0 : _e.extra) && typeof this.extras[0].extra !== 'string');
                if (!extrasPoblados) {
                    logger.warn(`Extras no poblados al calcular total del viaje, intentando poblar: ${this._id}`);
                    yield this.populate('extras.extra');
                    extrasPoblados = this.populated('extras.extra'); // Verificar de nuevo
                }
                if (extrasPoblados) {
                    totalExtras = this.extras.reduce((sum, extraItem) => {
                        var _a;
                        const valorExtra = Number((_a = extraItem === null || extraItem === void 0 ? void 0 : extraItem.extra) === null || _a === void 0 ? void 0 : _a.valor);
                        const cantidadExtra = Number(extraItem === null || extraItem === void 0 ? void 0 : extraItem.cantidad);
