...
tramoSchema.methods.getTarifaVigente = function (fecha = new Date(), tipo) {
    if (tipo) {
    ...
    try {
        ...
        if (this.origen && this.destino && (!this.distancia || this.distancia === 0)) {
            ...
            if (origenSite?.location?.coordinates?.length === 2 &&
                ...
                try {
                    const distanciaKm = await calcularDistanciaRuta(origenSite.location.coordinates, destinoSite.location.coordinates);
...
tramoSchema.pre('save', async function (next) {
    try {
        ...
        if (this.tarifasHistoricas && this.tarifasHistoricas.length > 1) {
            ...
            const tarifasOrdenadas = [...this.tarifasHistoricas].sort((a, b) => a.vigenciaDesde.getTime() - b.vigenciaDesde.getTime());
...
tramoSchema.virtual('descripcion').get(async function () {
    try {
        ...
        const nombreOrigen = this.origen?.nombre || 'ID Origen Desc.';
        const nombreDestino = this.destino?.nombre || 'ID Destino Desc.';
        const nombreCliente = this.cliente?.nombre || 'ID Cliente Desc.';
...
const Tramo = model('Tramo', tramoSchema);
