...
const createVehiculo = async (vehiculoData) => {
    ...
    try {
        ...
        await Empresa.findByIdAndUpdate(vehiculoData.empresa, { $push: { flota: vehiculoGuardado._id } }, { session });
...
const updateVehiculo = async (id, vehiculoData) => {
    ...
    try {
        ...
        if (vehiculoData.empresa && vehiculoData.empresa.toString() !== vehiculo.empresa.toString()) {
            ...
            await Empresa.findByIdAndUpdate(vehiculo.empresa, { $pull: { flota: vehiculo._id } }, { session });
            // Agregar a la nueva empresa
            await Empresa.findByIdAndUpdate(vehiculoData.empresa, { $push: { flota: vehiculo._id } }, { session });
        }
        // Actualizamos el vehículo dentro de la transacción
        const vehiculoActualizado = await Vehiculo.findByIdAndUpdate(id, vehiculoData, { new: true, runValidators: true, session });
            ...
            if (result.hasWriteErrors()) {
                result.getWriteErrors().forEach(err => {
                    ...
                    errores.push({
                        ...
                        code: err.code,
                        data: err.op || 'No disponible' // Contiene la operación fallida
                    });
                });
...
export { getAllVehiculos, getVehiculosByEmpresa, getVehiculoById, createVehiculo, updateVehiculo, deleteVehiculo, getVehiculosConVencimientos, getVehiculosVencidos, createVehiculosBulk };
