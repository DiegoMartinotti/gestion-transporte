...
export class ExcelTemplateService {
     ...
     */
    static async generateMissingDataTemplates(res: Response, importacion: IImportacionTemporal): Promise<void> {
        try {
            ...
            if (importacion.failureDetails.missingSites.count > 0) {
                logger.info(`Generando hoja de Sites faltantes: ${importacion.failureDetails.missingSites.count} sites`);
                await this.addMissingSitesSheet(workbook, importacion.failureDetails.missingSites.details, cliente.nombre);
                hasSheets = true;
            }

            // Generar hoja de Personal faltante
            if (importacion.failureDetails.missingPersonal.count > 0) {
                await this.addMissingPersonalSheet(workbook, importacion.failureDetails.missingPersonal.details);
                hasSheets = true;
            }

            // Generar hoja de Vehículos faltantes
            if (importacion.failureDetails.missingVehiculos.count > 0) {
                await this.addMissingVehiculosSheet(workbook, importacion.failureDetails.missingVehiculos.details);
                hasSheets = true;
            }

            // Generar hoja de Tramos faltantes
            if (importacion.failureDetails.missingTramos.count > 0) {
                await this.addMissingTramosSheet(workbook, importacion.failureDetails.missingTramos.details, cliente.nombre);
                hasSheets = true;
            }
            ...
            instructionsSheet.addRow(['']);
            instructionsSheet.addRow(['Este archivo contiene plantillas pre-rellenadas con los datos que faltan en su importación.']);
            instructionsSheet.addRow(['']);
            instructionsSheet.addRow(['INSTRUCCIONES:']);
            instructionsSheet.addRow(['1. Complete TODOS los campos marcados con * en cada hoja']);
            instructionsSheet.addRow(['2. Use las hojas de referencia disponibles para verificar los valores válidos']);
            instructionsSheet.addRow(['3. Importe cada hoja por separado usando el sistema correspondiente:']);
            ...
            instructionsSheet.addRow(['4. Revise la hoja "Viajes Fallidos" para identificar errores específicos']);
            instructionsSheet.addRow(['5. Una vez importados todos los datos, reintente la importación de viajes']);
            instructionsSheet.addRow(['']);
            instructionsSheet.addRow(['IMPORTANTE: No modifique los nombres de las columnas ni elimine las hojas de referencia']);
     ...
     */
    private static async addMissingSitesSheet(workbook: ExcelJS.Workbook, sitesNames: string[], clienteName: string): Promise<void> {
     ...
     */
    private static async addMissingPersonalSheet(workbook: ExcelJS.Workbook, personalDnis: string[]): Promise<void> {
     ...
     */
    private static async addMissingVehiculosSheet(workbook: ExcelJS.Workbook, vehiculosDominios: string[]): Promise<void> {
        ...
        for (const trip of failedTrips) {
            ...
            try {
                ...
                if (tripData.chofer && Types.ObjectId.isValid(tripData.chofer)) {
                    const choferPersonal = await Personal.findById(tripData.chofer).select('nombre apellido').lean();
                    choferNombre = choferPersonal ? `${choferPersonal.nombre} ${choferPersonal.apellido}` : `ID: ${tripData.chofer}`;
                } else {
    ...
    static async processCorrectionTemplate(fileBuffer: Buffer, importacion: IImportacionTemporal): Promise<any> {
        try {
            ...
            if (tramosSheet) {
                ...
                resultados.tramos = await this.processTramosSheet(tramosSheet, importacion.cliente.toString());
            }
    ...
    private static async processSitesSheet(worksheet: ExcelJS.Worksheet, clienteId: string): Promise<any> {
        ...
        worksheet.eachRow((row, rowNumber) => {
            ...
            const siteData = {
                ...
                coordenadas: row.getCell(7).value && row.getCell(8).value ? {
    ...
    private static async processTramosSheet(worksheet: ExcelJS.Worksheet, clienteId: string): Promise<any> {
        ...
        try {
            ...
            // Extraer los resultados de la respuesta
            const resultados = responseData?.data?.resultados || responseData?.resultados || responseData || { exitosos: 0, errores: [] };
