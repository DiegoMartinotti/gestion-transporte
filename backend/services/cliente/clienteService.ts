...
class ClienteService extends BaseService<ICliente> {
  ...
  async getByNombre(nombre: string): Promise<ICliente | null> {
    ...
    try {
      ...
      const cliente = await this.model
        .findOne({
          nombre: nombre.trim(),
        })
        .lean()
   ...
   * @param options - Opciones de transacción
   */
  protected async afterCreate(cliente: ICliente, _options: TransactionOptions = {}): Promise<void> {
   ...
   * @param options - Opciones de transacción
   */
  protected async afterUpdate(cliente: ICliente, _options: TransactionOptions = {}): Promise<void> {
