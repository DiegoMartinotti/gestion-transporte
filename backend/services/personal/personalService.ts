...
class PersonalService extends BaseService<IPersonal> {
   ...
   */
  private validateTextFields(data: Partial<IPersonal>): void {
  ...
  /**
   * Valida campos de identificación
   */
  private validateIdentificationFields(data: Partial<IPersonal>): void {
  ...
  /**
   * Valida campos de contacto y fecha
   */
  private validateContactAndDateFields(data: Partial<IPersonal>): void {
  ...
  /**
   * Valida campos relacionados con empresa
   */
  private async validateCompanyRelatedFields(data: Partial<IPersonal>): Promise<void> {
    ...
    if (data.numeroLegajo && data.empresa) {
      await this.validateUniqueLegajo(
        data.numeroLegajo,
        data.empresa as Types.ObjectId | string,
        data._id?.toString()
      );
    }
  }
  ...
  protected async validateData(data: Partial<IPersonal>): Promise<void> {
    ...
    this.validateIdentificationFields(data);

    // Validar campos de contacto y fecha
    this.validateContactAndDateFields(data);

    // Validar campos relacionados con empresa
    await this.validateCompanyRelatedFields(data);
   ...
   */
  private async validateUniqueLegajo(
    numeroLegajo: string,
  ...
  async getPersonalByTipo(
    ...
    try {
      ...
      const filtros: FilterQuery<IPersonal> = {
        ...(opciones.filtros as Record<string, unknown>),
        tipo: tipo,
      };
  ...
  async getPersonalConVencimientos(diasLimite: number = 30, empresaId?: string) {
    ...
    try {
      ...
      const personal = await this.model.find(filtros).populate('empresa', 'nombre').lean().exec();
   ...
   */
  /**
   * Construye el pipeline de agregación para estadísticas
   */
  private buildEstadisticasPipeline(matchFilter: Record<string, unknown>) {
    return [
      { $match: matchFilter },
      {
        $group: {
          _id: { tipo: '$tipo', activo: '$activo' },
          count: { $sum: 1 },
        },
      },
      {
        $group: {
          _id: '$_id.tipo',
          activos: { $sum: { $cond: [{ $eq: ['$_id.activo', true] }, '$count', 0] } },
          inactivos: { $sum: { $cond: [{ $eq: ['$_id.activo', false] }, '$count', 0] } },
          total: { $sum: '$count' },
        },
      },
      {
  ...
  async getEstadisticas(empresaId?: string) {
    ...
    try {
      ...
      const pipeline = this.buildEstadisticasPipeline(matchFilter);
  ...
  protected async afterCreate(
    ...
    this.logInfo(
      `Personal creado exitosamente: ${personal.nombre} ${personal.apellido} (DNI: ${personal.dni})`
    );
  ...
  protected async afterUpdate(
    ...
    this.logInfo(
      `Personal actualizado exitosamente: ${personal.nombre} ${personal.apellido} (${personal._id})`
    );
  }
  ...
  protected async beforeDelete(
    ...
    this.logWarn(
      `Preparando eliminación del personal: ${personal.nombre} ${personal.apellido} (${personal._id})`
    );
