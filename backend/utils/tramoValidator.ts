 ...
 */
interface Tramo {
  origen: string | object; // Puede ser un ID de MongoDB u otro tipo
  destino: string | object; // Puede ser un ID de MongoDB u otro tipo
  tipo?: string;
  metodoCalculo?: string;
  cliente?: string | object; // Puede ser un ID u otro tipo
...
const fechasSuperpuestas = (
  ...
  logger.debug(
    `Rango 1: ${f1Desde.toISOString().split('T')[0]} - ${f1Hasta.toISOString().split('T')[0]}`
  );
  logger.debug(
    `Rango 2: ${f2Desde.toISOString().split('T')[0]} - ${f2Hasta.toISOString().split('T')[0]}`
  );
...
const generarTramoId = (tramo: Tramo): string => {
  ...
  logger.debug(
    `[DIAGNÓSTICO] generarTramoId - Datos: origen=${tramo.origen}, destino=${tramo.destino}, tipo=${tipo}, metodo=${metodo}`
  );
...
const sonTramosIguales = (tramo1: Tramo, tramo2: Tramo): boolean => {
  ...
  const sonIguales =
    String(tramo1.origen) === String(tramo2.origen) &&
    String(tramo1.destino) === String(tramo2.destino) &&
    tipo1 === tipo2 &&
...
const debugTramo = (tramo: Tramo, etiqueta: string = 'Tramo'): void => {
  ...
  logger.debug(
    `Tipo: ${tramo.tipo} (normalizado: ${tramo.tipo ? tramo.tipo.toUpperCase() : 'TRMC'})`
  );
...
export { normalizarFecha, fechasSuperpuestas, generarTramoId, sonTramosIguales, debugTramo, Tramo };
