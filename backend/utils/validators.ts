...
const validators = {
   ...
   */
  esObjectIdValido: (id: string): boolean => {
    if (!id) return false;
   ...
   * @returns true si es válido
   */
  esDominioValido: (dominio: string): boolean => {
   ...
   */
  normalizarDominio: (dominio: string): string => {
    if (!dominio) return '';
   ...
   */
  esNumeroPositivo: (valor: number | string): boolean => {
   ...
   */
  stringABooleano: (valor: string | boolean): boolean => {
