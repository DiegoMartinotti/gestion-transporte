...
const { actualizarEstadosPartidaBulk } = require('../utils/estadoPartida');
 ...
 */
ordenCompraSchema.methods.calcularImporteTotal = async function() {
 ...
 */
ordenCompraSchema.post('save', async function(doc) {
  try {
    if (doc.isModified('viajes')) {
      const viajeIds = doc.viajes.map(v => v.viaje);
