...
/**
 * Interfaz que representa un documento de Extra en la base de datos
 */
...
const extraSchema = new Schema(
  {
    ...
    valor: {
      ...
      min: 0,
      get: (v: number): string => parseFloat(v.toString()).toFixed(2),
      set: (v: number): number => parseFloat(parseFloat(v.toString()).toFixed(2)),
    },
  },
  {
    timestamps: true,
...
extraSchema.pre('save', async function (this: IExtra, next) {
  try {
    ...
    const extrasExistentes = await (this.constructor as unknown).find({
      tipo: this.tipo,
    ...
    for (const extra of extrasExistentes) {
      ...
      if (hayConflicto) {
        throw new Error(
          `Ya existe un extra del mismo tipo para este cliente con fechas que se superponen.`
        );
      }
    }
