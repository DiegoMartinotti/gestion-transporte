  ...
  for (const indexDef of recommendedIndexes) {
    try {
      ...
      const indexExists = existingIndexes.some(idx => {
        ...
        return keys.length === defKeys.length && 
               keys.every(k => defKeys.includes(k));
      });
 ...
 */
export async function analyzeIndexUsage(collectionName: string): Promise<any> {
  const db = mongoose.connection.db;
 ...
 */
export async function getIndexSizes(): Promise<Record<string, any>> {
  const db = mongoose.connection.db;
