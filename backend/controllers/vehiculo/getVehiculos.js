...
const getVehiculos = async (req, res) => {
  ...
  try {
    ...
    if (opciones.vencimientoProximo) {
      ...
      resultado = await vehiculoService.getVehiculosConVencimientos(opciones.diasVencimiento);
      ...
      resultado = await vehiculoService.getVehiculosVencidos();
      logger.info(`Obtenidos ${resultado.length} vehículos con documentos vencidos`);
    } else {
      // Consulta estándar con filtros
      resultado = await vehiculoService.getAllVehiculos(opciones);
