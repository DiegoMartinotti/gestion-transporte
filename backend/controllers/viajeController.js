 ...
 */
exports.iniciarBulkImportViajes = async (req, res) => {
    ...
    try {
        ...
        const importacion = new ImportacionTemporal({
            ...
            failureDetails: { // Inicializar detalles de fallos
        ...
        const clienteDoc = await Cliente.findById(clienteId).session(session).lean(); // Usar sesión
        if (!clienteDoc) {
            // Actualizar estado de importación a 'failed' si el cliente no existe
            // Guardar el estado 'failed' antes de salir
            await ImportacionTemporal.findByIdAndUpdate(importacionId, { status: 'failed', failureDetails: { invalidData: { count: 1, details: [`Cliente con ID ${clienteId} no encontrado`] }} }, { session });
            await session.commitTransaction(); // Commit para guardar el estado 'failed'
            session.endSession();
        ...
        const dtsExistentes = await Viaje.find({ cliente: clienteId, dt: { $in: [...allDtSet] } }).select('dt').session(session).lean(); // Usar sesión
        ...
        choferesDocs.forEach(c => {
            ...
            if (legajoKey) choferMap.set(legajoKey, c);
            if (dniKey) choferMap.set(dniKey, c); // Puede sobreescribir si DNI y Legajo coinciden con identificador, ok
        });

        // Buscar Vehículos por Dominio (patente)
        const vehiculosDocs = await Vehiculo.find({ dominio: { $in: allPatentes } }).session(session).lean(); // Usar sesión
        ...
        sitesDocs.forEach(s => {
            ...
            if (keySite) siteMap.set(keySite, s);
            // Asegurar que no sobreescribimos con una clave vacía y que añadimos por nombre si es diferente
            if (keyNombre && keyNombre !== keySite) siteMap.set(keyNombre, s);
            else if (keyNombre && !keySite) siteMap.set(keyNombre, s); // Añadir por nombre si 'Site' estaba vacío
        });
            ...
            try {
                ...
                tramosCoincidentes.forEach(tramoDoc => {
                    if (!tramoDoc || !Array.isArray(tramoDoc.tarifasHistoricas) || tramoDoc.tarifasHistoricas.length === 0) {
                                logger.warn(`Tramo ID: ${tramoDoc?._id} no tiene tarifas históricas válidas. Saltando este tramo.`);
                                return;
                    }
                    const tarifasVigentesDelTramo = tramoDoc.tarifasHistoricas.filter(tarifa => {
                        ...
                        try {
                                    if (!tarifa.vigenciaDesde || !tarifa.vigenciaHasta) { throw new Error('Fechas de vigencia faltantes'); }
                            vigenciaDesdeDate = new Date(tarifa.vigenciaDesde);
                            vigenciaHastaDate = new Date(tarifa.vigenciaHasta);
                                    if (isNaN(vigenciaDesdeDate) || isNaN(vigenciaHastaDate)) { throw new Error('Fechas de vigencia inválidas'); }
                        } catch (e) {
                ...
                if (tieneDatosFaltantes) {
                    // Guardar para corrección posterior
                     viajesPendientesCorreccion.push({
                        ...viajeData, // Datos originales
                        originalIndex: indiceOriginal,
                        missingReasons: [...missingReasons] // Guardar las razones específicas
                     });
                ...
                const viajeParaGuardar = new Viaje({
                    ...
                    await viajeParaGuardar.save({ session });
        ...
        const importacionActualizada = await ImportacionTemporal.findById(importacionId).session(session);
        ...
        await importacionActualizada.save({ session });
        ...
        if (importacionId) {
            try {
                ...
                await ImportacionTemporal.findByIdAndUpdate(importacionId, {
                     ...
                     failureDetails: { ... (importacionActualizada?.failureDetails || {}), internalProcessingError: { count: 1, details: ['Error fatal en el servidor: ' + error.message] } }
                 });
            } catch (updateError) {
 ...
 */
exports.descargarPlantillaCorreccion = async (req, res) => {
    ...
    try {
        ...
        res.setHeader(
            'Content-Disposition',
            `attachment; filename="Plantilla_Correccion_${templateType}_${importId}.xlsx"`
        );
 ...
 * @param {Object} res - Objeto de respuesta Express
 */
exports.procesarPlantillaCorreccion = async (req, res) => {
    const { importId, templateType } = req.params;
    ...
    try {
        const importacion = await ImportacionTemporal.findById(importId).session(session);
            ...
            try {
                switch (templateType) {
                    case 'Site':
                        ...
                        if (!rowData.Site || !rowData.location?.coordinates?.[0] || !rowData.location?.coordinates?.[1]) {
        ...
        switch (templateType) {
            case 'Site':
                ...
                bulkCreateResult = await siteController.createSitesBulk(extractedData, { session });
                break;
            case 'Personal':
                 // Asumiendo que personalService.createPersonalBulk existe
                 bulkCreateResult = await personalController.createPersonalBulk(extractedData, { session });
                break;
            case 'Vehiculo':
                 // Usar el servicio existente
                 bulkCreateResult = await vehiculoService.createVehiculosBulk(extractedData, { session }); // Pasar session si el servicio lo soporta
                break;
            case 'Tramo':
                 ...
                 bulkCreateResult = await tramoService.createTramosBulk(extractedData, { session });
                break;
        }
 ...
 * @param {Object} res - Objeto de respuesta Express
 */
exports.reintentarImportacionViajes = async (req, res) => {
    ...
    try {
        ...
        if (!clienteDoc) throw new Error('Cliente no encontrado durante el reintento.'); // Error interno si esto pasa

        const allDtSetRetry = new Set(viajesParaReintentar.map(v => String(v.data?.dt || '').trim()).filter(dt => dt));
        const allChoferIdentifiersRetry = [...new Set(viajesParaReintentar.map(v => String(v.data?.chofer || '').trim()).filter(c => c))];
        const allPatentesRawRetry = viajesParaReintentar.flatMap(v => typeof v.data?.vehiculo === 'string' ? v.data.vehiculo.split(',') : [])
                                          .map(p => String(p || '').trim().toUpperCase()).filter(p => p);
        const allPatentesRetry = [...new Set(allPatentesRawRetry)];
        const allSiteNombresRawRetry = viajesParaReintentar.flatMap(v => [String(v.data?.origenNombre || '').trim(), String(v.data?.destinoNombre || '').trim()])
                                             .filter(n => n);
        ...
        const dtsInicialesExitosos = await Viaje.find({ cliente: clienteId, dt: { $in: [...allDtSetRetry] } }).select('dt').session(session).lean();
        ...
        const choferesDocsRetry = await Personal.find({
            activo: true,
        ...
        choferesDocsRetry.forEach(c => {
            if (c.legajo) choferMapRetry.set(String(c.legajo).trim(), c);
            if (c.dni) choferMapRetry.set(String(c.dni).trim(), c);
        });

        const vehiculosDocsRetry = await Vehiculo.find({ dominio: { $in: allPatentesRetry } }).session(session).lean();
        const vehiculoMapRetry = new Map(vehiculosDocsRetry.map(v => [String(v.dominio).trim().toUpperCase(), v]));

        const sitesDocsRetry = await Site.find({
        ...
        sitesDocsRetry.forEach(s => {
            const keySite = String(s.Site || '').trim().toLowerCase();
            const keyNombre = String(s.nombre || '').trim().toLowerCase();
            if (keySite) siteMapRetry.set(keySite, s);
            if (keyNombre && keyNombre !== keySite) siteMapRetry.set(keyNombre, s);
            else if (keyNombre && !keySite) siteMapRetry.set(keyNombre, s);
        });
        ...
        for (const failedTrip of viajesParaReintentar) {
            ...
            try {
                ...
                if (!dtNormalizado) throw new Error('INVALID_DATA: El campo DT es requerido');
                if (dtsExistentesSetRetry.has(dtNormalizado)) {
                    // Si sigue siendo duplicado, registrarlo como fallo final
                    throw new Error(`DUPLICATE_DT: El DT ${dtNormalizado} ya existe para este cliente`);
                }
                if (!viajeData.fecha || isNaN(new Date(viajeData.fecha).getTime())) throw new Error(`INVALID_DATA: Fecha inválida: ${viajeData.fecha}`);
                const fechaViaje = new Date(viajeData.fecha);

                if (!viajeData.origenNombre || !viajeData.destinoNombre) throw new Error('INVALID_DATA: Los campos Origen y Destino son requeridos');
                ...
                if (!origenNombreNorm || !destinoNombreNorm) throw new Error('INVALID_DATA: Los campos Origen y Destino son requeridos');

                const origenDoc = siteMapRetry.get(origenNombreNorm);
                const destinoDoc = siteMapRetry.get(destinoNombreNorm);
                if (!origenDoc) throw new Error(`MISSING_SITE: Sitio de Origen '${viajeData.origenNombre}' no encontrado (reintento).`);
                if (!destinoDoc) throw new Error(`MISSING_SITE: Sitio de Destino '${viajeData.destinoNombre}' no encontrado (reintento).`);
                origenId = origenDoc._id;
                destinoId = destinoDoc._id;

                if (!viajeData.chofer) throw new Error('INVALID_DATA: El campo Chofer (Legajo/DNI) es requerido');
                const choferIdentifierNorm = String(viajeData.chofer || '').trim();
                if (!choferIdentifierNorm) throw new Error('INVALID_DATA: El campo Chofer (Legajo/DNI) es requerido');
                const choferDoc = choferMapRetry.get(choferIdentifierNorm);
                if (!choferDoc) throw new Error(`MISSING_PERSONAL: Chofer con identificador '${viajeData.chofer}' no encontrado o inactivo (reintento).`);
                choferId = choferDoc._id;

                if (!viajeData.vehiculo || typeof viajeData.vehiculo !== 'string' || viajeData.vehiculo.trim() === '') throw new Error('INVALID_DATA: El campo Vehículos (patentes) es requerido y no puede estar vacío');
                const vehiculoInput = String(viajeData.vehiculo || '').trim();
                if (!vehiculoInput) throw new Error('INVALID_DATA: El campo Vehículos (patentes) es requerido y no puede estar vacío');
                const patentesArrayNorm = vehiculoInput.split(',').map(p => String(p || '').trim().toUpperCase()).filter(p => p);
                if (patentesArrayNorm.length === 0) throw new Error('INVALID_DATA: Debe ingresar al menos una patente válida en Vehículos');

                vehiculosParaViaje = [];
                for (let j = 0; j < patentesArrayNorm.length; j++) {
                    const patenteNorm = patentesArrayNorm[j];
                    const vehiculoDoc = vehiculoMapRetry.get(patenteNorm);
                    if (!vehiculoDoc) throw new Error(`MISSING_VEHICULO: Vehículo con patente '${patenteNorm}' no encontrado (reintento).`);
                    vehiculosParaViaje.push({ vehiculo: vehiculoDoc._id, posicion: j + 1 });
                }

                // Re-buscar tramo y tarifa
                const tramosCoincidentes = await Tramo.find({ cliente: clienteId, origen: origenId, destino: destinoId }).lean(); // No necesita session aquí
                if (!tramosCoincidentes || tramosCoincidentes.length === 0) throw new Error(`MISSING_TRAMO: No se encontró tramo para Cliente ID: ${clienteId}, Origen: ${viajeData.origenNombre}, Destino: ${viajeData.destinoNombre} (reintento).`);
                ...
                if (todasLasTarifasVigentes.length === 0) throw new Error(`MISSING_TRAMO: No se encontró tarifa vigente para tramo Origen: ${viajeData.origenNombre}, Destino: ${viajeData.destinoNombre} en fecha: ${viajeData.fecha} (reintento).`);
                ...
                if (!tarifaSeleccionada) throw new Error(`MISSING_TRAMO: No se pudo seleccionar tarifa vigente válida (valor/tipo inválido) para fecha: ${viajeData.fecha} (reintento).`);
                ...
                logger.info(`Viaje DT ${dtNormalizado} importado exitosamente (reintento).`);
                 ...
                 logger.warn(`Error reintentando viaje #${indiceOriginal + 1} (DT: ${viajeData?.dt}): ${error.message}`);
                 // Clasificar error final
                 let reason = 'RETRY_FAILED'; // Razón genérica de fallo en reintento
                 let details = error.message;
                 // Re-clasificar si es posible
                 if (error.message.startsWith('DUPLICATE_DT:')) { reason = 'DUPLICATE_DT'; details = error.message.substring('DUPLICATE_DT:'.length).trim(); }
                 else if (error.message.startsWith('MISSING_SITE:')) { reason = 'MISSING_SITE'; details = error.message.substring('MISSING_SITE:'.length).trim(); }
                 else if (error.message.startsWith('MISSING_PERSONAL:')) { reason = 'MISSING_PERSONAL'; details = error.message.substring('MISSING_PERSONAL:'.length).trim(); }
                 else if (error.message.startsWith('MISSING_VEHICULO:')) { reason = 'MISSING_VEHICULO'; details = error.message.substring('MISSING_VEHICULO:'.length).trim(); }
                 else if (error.message.startsWith('MISSING_TRAMO:')) { reason = 'MISSING_TRAMO'; details = error.message.substring('MISSING_TRAMO:'.length).trim(); }
                 else if (error.message.startsWith('INVALID_DATA:')) { reason = 'INVALID_DATA'; details = error.message.substring('INVALID_DATA:'.length).trim(); }
        ...
        try {
            await ImportacionTemporal.findByIdAndUpdate(importId, { status: 'failed' });
        } catch (updateError) {
 ...
 * @param {Object} res - Objeto de respuesta Express
 */
exports.descargarFallbackViajes = async (req, res) => {
    const { importId } = req.params;
    ...
    try {
        ...
        const dataRows = viajesParaDescargar.map(fail => {
            const rowData = {
                ...
                dt: fail.data?.dt || fail.dt || 'N/A', 
                fecha: fail.data?.fecha || fail.fecha || 'N/A', 
                origenNombre: fail.data?.origenNombre || fail.origenNombre || 'N/A',
                destinoNombre: fail.data?.destinoNombre || fail.destinoNombre || 'N/A',
                chofer: fail.data?.chofer || fail.chofer || 'N/A',
                vehiculo: fail.data?.vehiculo || fail.vehiculo || 'N/A',
                paletas: (fail.data?.paletas !== undefined ? fail.data.paletas : fail.paletas) ?? 'N/A', // Usar ?? para manejar null/undefined
...
const addFormatsSheet = (workbook, templateType, headers) => {
    ...
    const formatRows = headers.map(h => {
        ...
        if (h.key === 'esDeposito') description = 'Indicar SI o NO.'; 
        else if (h.key === 'activar') description = 'Indicar SI para activar personal existente inactivo, o NO/vacío para crear uno nuevo.';
        // *** Personal ***
        else if (templateType === 'Personal' && h.key === 'tipo') description = 'Ej: Conductor, Administrativo, Mecánico, Supervisor, Otro.';
        else if (templateType === 'Personal' && h.key === 'empresa') description = 'ID o Nombre exacto de la hoja Empresas.';
        // *** Vehiculo ***
        else if (templateType === 'Vehiculo' && h.key === 'anio') description = 'Año de fabricación (número).';
        else if (templateType === 'Vehiculo' && h.key === 'empresa') description = 'ID o Nombre exacto de la hoja Empresas.';
         // *** Tramo/Tarifa ***
        else if (templateType === 'Tramo' && h.key === 'tipo') description = 'Indicar TRMC o TRMI.';
        else if (templateType === 'Tramo' && h.key === 'metodoCalculo') description = 'Indicar Kilometro, Palet o Fijo.';
        else if (templateType === 'Tramo' && (h.key === 'valor' || h.key === 'valorPeaje')) description = 'Valor numérico (usar punto como decimal).';
        else if (templateType === 'Tramo' && (h.key === 'vigenciaDesde' || h.key === 'vigenciaHasta')) description = 'Fecha en formato YYYY-MM-DD.';
        // *** Site ***
        else if (h.key === 'longitud' || h.key === 'latitud') description = 'Coordenada geográfica (número decimal, usar punto).'; 
        // Default
        else description = `Dato para ${cleanHeader}.`;
