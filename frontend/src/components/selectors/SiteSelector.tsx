...
import { Select, Group, Text, Badge, Stack, Tooltip, ActionIcon, Loader } from '@mantine/core';
...
// Función utilitaria para calcular distancia
...
// Componente extraído para renderizar opciones del selector
interface SiteOptionProps {
  site: SiteWithDistance;
  showCoordinates: boolean;
  showDistance: boolean;
  onOpenMaps: (site: Site, event: React.MouseEvent) => void;
}

function SiteOption({ site, showCoordinates, showDistance, onOpenMaps }: SiteOptionProps) {
  ...
  return (
    <Group justify="space-between" wrap="nowrap">
      ...
      <Tooltip label="Ver en Google Maps">
        <ActionIcon size="sm" variant="subtle" color="blue" onClick={(e) => onOpenMaps(site, e)}>
...
// Hook personalizado para manejo del estado del selector
function useSiteSelector({
  clienteId,
  showDistance,
  fromLocation,
  debouncedSearch,
        ...
        const sitesWithDistance: SiteWithDistance[] = sitesData
          ...
          .map((site) => ({
            ...
            distance: calculateDistance(
              fromLocation.lat,
              fromLocation.lng,
              site.coordenadas?.lat || 0,
              site.coordenadas?.lng || 0
            ),
          }));
  ...
  }, [debouncedSearch, clienteId, showDistance, fromLocation]);
...
export default function SiteSelector({
  ...
  const { sites, loading } = useSiteSelector({
    clienteId,
    showDistance,
    fromLocation,
    debouncedSearch,
  ...
  const openGoogleMaps = useCallback((site: Site, event: React.MouseEvent) => {
  ...
  const renderSelectOption = ({
    ...
    option: { value: string; label: string; site: SiteWithDistance };
  }) => {
    return (
      <SiteOption
        site={option.site}
