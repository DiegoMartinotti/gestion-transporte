...
import { Paper, Title, Select, Group, Card, Text, Badge, Stack, Alert } from '@mantine/core';
...
function AlertasInfo({
  ...
  const origenNombre = origenes.find((o) => o._id === selectedOrigen)?.nombre;

  if (selectedOrigen && destinosDisponibles.length > 0 && !selectedDestino) {
    return (
      <Alert color="blue" variant="light">
        <Text size="sm">
          Destinos disponibles desde <strong>{origenNombre}</strong>: {destinosDisponibles.length}
        </Text>
      </Alert>
    );
  }

  if (selectedOrigen && destinosDisponibles.length === 0) {
    return (
      <Alert color="orange" variant="light">
        <Text size="sm">
          No hay destinos disponibles desde <strong>{origenNombre}</strong>
        </Text>
      </Alert>
    );
  }
  ...
  return (
    <>
      <Select
        ...
        data={origenes.map((origen) => ({ value: origen._id, label: origen.nombre }))}
      ...
      <Select
        label="Destino"
        placeholder={
          selectedOrigen ? 'Seleccione el punto de destino' : 'Primero seleccione un origen'
        }
        value={selectedDestino}
        onChange={onDestinoChange}
        data={destinosDisponibles.map((destino) => ({ value: destino._id, label: destino.nombre }))}
...
export const TramosSelector: React.FC<TramosSelectorProps> = ({ onTramoSelect, selectedTramo }) => {
  ...
  const { data: tramos = [], isLoading } = useQuery({
    ...
    select: (data) => {
      const processedTramos = Array.isArray(data) ? data : (data as { data?: Tramo[] })?.data || [];
  ...
  const tramoSeleccionado = useMemo(() => {
    ...
    return (
      tramos.find(
        (tramo: Tramo) =>
          tramo.origen?._id === selectedOrigen && tramo.destino?._id === selectedDestino
      ) || null
    );
  ...
  return (
    <Paper p="md" withBorder>
      ...
      <Stack gap="md">
        ...
        />
        {tramoSeleccionado && <TramoInfoCard tramo={tramoSeleccionado} />}
        <AlertasInfo
          selectedOrigen={selectedOrigen}
          selectedDestino={selectedDestino}
          origenes={origenes}
