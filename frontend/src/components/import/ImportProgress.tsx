...
const calculateProcessingStats = (
  ...
  const estimatedCompletion = speed > 0 ? new Date(now.getTime() + estimatedSeconds * 1000) : null;
...
const getStatusColor = (errors: number, warnings: number): string => {
  if (errors > 0) return 'red';
  if (warnings > 0) return 'yellow';
  return 'green';
};

const getStatusIcon = (errors: number, warnings: number) => {
...
export const ImportProgress: React.FC<ImportProgressProps> = ({
      ...
      <Stack gap="sm">
        <Group justify="space-between">
          ...
          <Group>
            ...
            <ActionIcon variant="subtle" onClick={() => setShowDetails(!showDetails)}>
          ...
          <Card withBorder>
            <Stack gap="xs">
              ...
              <Group justify="space-between">
                ...
                <Badge variant="light">{stats.speed} registros/seg</Badge>
              </Group>
              ...
              <Group justify="space-between">
                ...
                <Badge variant="light">{Math.round((currentBatch / totalBatches) * 100)}%</Badge>
              </Group>
            </Stack>
          </Card>
        </SimpleGrid>
        ...
        <Timeline active={-1} bulletSize={24} lineWidth={2}>
          ...
          {!isProcessing && processed === total && (
            <Timeline.Item bullet={<IconCheck size={12} />} title="Proceso completado" c="green">
              <Text c="dimmed" size="sm">
                Importación finalizada en {Math.floor(stats.elapsedTime / 60)}m{' '}
                {Math.floor(stats.elapsedTime % 60)}s
              </Text>
            </Timeline.Item>
          )}
        </Timeline>
  ...
  const renderAlerts = () => {
    if (errors > 0 && !isProcessing) return renderErrorAlert();
    if (warnings > 0 && errors === 0 && !isProcessing) return renderWarningAlert();
  ...
  return (
    <Stack gap="md">
      ...
      <Collapse in={showDetails}>{renderDetailsSection()}</Collapse>
