...
export default function SiteMap({
  ...
  useEffect(() => {
    ...
    filteredSites.forEach((site) => {
      ...
      // Info window content
      const clienteNombre = typeof site.cliente === 'string' 
  ...
  useEffect(() => {
    ...
    const selectedMarker = markers.find(marker => {
      ...
      return position.lat() === selectedSite.coordenadas?.lat && 
    ...
    if (selectedMarker) {
      ...
      // Abrir info window del site seleccionado
      const clienteNombre = typeof selectedSite.cliente === 'string' 
        ? clientes.find(c => c._id === selectedSite.cliente)?.nombre || 'Cliente no encontrado'
        : selectedSite.cliente?.nombre || 'Sin cliente';

      const infoContent = `
        <div style="max-width: 300px; padding: 8px;">
          <h3 style="margin: 0 0 8px 0; color: #228be6;">${selectedSite.nombre}</h3>
          <p style="margin: 4px 0; color: #666;"><strong>Cliente:</strong> ${clienteNombre}</p>
          <p style="margin: 4px 0; color: #666;"><strong>Dirección:</strong> ${selectedSite.direccion}</p>
          <p style="margin: 4px 0; color: #666;"><strong>Ciudad:</strong> ${selectedSite.localidad || selectedSite.ciudad || 'N/A'}</p>
          <p style="margin: 4px 0; color: #666;"><strong>Provincia:</strong> ${selectedSite.provincia}</p>
          <p style="margin: 4px 0; color: #666;">
            <strong>Estado:</strong> 
            <span style="color: ${selectedSite.activo === false ? '#dc3545' : '#28a745'};">
              ${selectedSite.activo === false ? 'Inactivo' : 'Activo'}
            </span>
          </p>
          <p style="margin: 8px 0 4px 0; font-size: 12px; color: #999;">
            Lat: ${selectedSite.coordenadas?.lat.toFixed(6)}, Lng: ${selectedSite.coordenadas?.lng.toFixed(6)}
          </p>
        </div>
      `;
        ...
        <Card withBorder>
          <Group justify="space-between" align="start">
            <Stack gap="xs" style={{ flex: 1 }}>
              <Group>
                <Text fw={600} size="lg">{selectedSite.nombre}</Text>
                <Badge
                  color={selectedSite.activo === false ? 'red' : 'green'}
                  variant="light"
                >
                  {selectedSite.activo === false ? 'Inactivo' : 'Activo'}
                </Badge>
              ...
              <Text size="sm" c="dimmed">
                <strong>Cliente:</strong> {
                  typeof selectedSite.cliente === 'string' 
                    ? clientes.find(c => c._id === selectedSite.cliente)?.nombre || 'Cliente no encontrado'
                    : selectedSite.cliente?.nombre || 'Sin cliente'
                }
              </Text>
              ...
              <Text size="sm">
                {selectedSite.direccion}, {selectedSite.localidad || selectedSite.ciudad}, {selectedSite.provincia}
              </Text>
              
              <Text size="xs" c="dimmed">
                Coordenadas: {selectedSite.coordenadas?.lat.toFixed(6)}, {selectedSite.coordenadas?.lng.toFixed(6)}
              </Text>
            </Stack>
