...
const VehiculoActions: React.FC<{
  vehiculo: Vehiculo;
  onEdit?: (vehiculo: Vehiculo) => void;
  onDelete?: (id: string) => void;
  onView?: (vehiculo: Vehiculo) => void;
}> = ({ vehiculo, onEdit, onDelete, onView }) => (
  <Group gap="xs">
    {onView && (
      <Tooltip label="Ver detalles">
        <ActionIcon variant="light" color="green" onClick={() => onView(vehiculo)}>
          <IconEye size={16} />
        </ActionIcon>
      </Tooltip>
    )}
    ...
    {onDelete && (
      <Tooltip label="Eliminar vehículo">
        <ActionIcon
          variant="light"
          color="red"
          onClick={() => vehiculo._id && onDelete(vehiculo._id)}
        >
          <IconTrash size={16} />
        </ActionIcon>
      </Tooltip>
...
const VehiculoBasicInfo: React.FC<{ vehiculo: Vehiculo }> = ({ vehiculo }) => (
  <>
...
const VehiculoDocumentacion: React.FC<{
  documentos: DocumentoEstado[];
  estadoGeneral: { estado: string; color: string; porcentaje: number };
}> = ({ documentos, estadoGeneral }) => (
  <>
    <Box>
...
export const VehiculoCard: React.FC<VehiculoCardProps> = ({
  ...
  return (
    <Card shadow="sm" padding="lg" radius="md" withBorder>
      ...
      <Stack gap="md" mt="md">
        <VehiculoBasicInfo vehiculo={vehiculo} />
        <VehiculoDocumentacion documentos={documentos} estadoGeneral={estadoGeneral} />
      </Stack>
    </Card>
  );
};
