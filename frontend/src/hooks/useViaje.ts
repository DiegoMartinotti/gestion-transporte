...
// Datos simulados extraídos para reducir líneas en función principal
const getViajesSimulados = (): Viaje[] => [
  {
    _id: '1',
    numeroViaje: '1001',
...
export function useViaje(viajeId: string) {
  ...
  const fetchViaje = useCallback(async () => {
    ...
    try {
      ...
      const viajesSimulados = getViajesSimulados();
    ...
    try {
      ...
      setViaje((prev) =>
        prev
          ? {
              ...prev,
              estado: nuevoEstado as Viaje['estado'],
