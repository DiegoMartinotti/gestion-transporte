...
export type ModalReturn<T = unknown> = ModalState<T> &
  ModalActions<T> & {
    onSuccess: () => void;
  };
 ...
 */
export function useModal<T = unknown>(config?: ModalConfig<T>): ModalReturn<T> {
  const { onOpen, onClose, onSuccess, resetOnClose = true } = config || {};
